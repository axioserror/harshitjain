...
const About = () => {
  ...
  return (
    <section className="text-white mb-5">
      <p className="mt-8 ml-8 text-xs bg-white/65 p-2 rounded-xl w-fit">⚪ About</p>
      <div className="flex flex-row justify-around  items-center mt-4 md:flex-wrap">
        <div>
          <img src={about} alt="harshit jain" className=" rounded-lg shadow-md shadow-slate-200/10 mb-6 " />
        </div>
        <div className="w-1/2">
          <h1 className="font-normal text-6xl font-poppins p-4 text-center">Harshit Jain</h1>
        ...
        {skills && skills.map((skill, i) => <li key={i} className="flex flex-row items-center font-inconsolata "> <RxTriangleRight />
         {skill}</li>)} 
        </ul>
        </div>
      </div>
